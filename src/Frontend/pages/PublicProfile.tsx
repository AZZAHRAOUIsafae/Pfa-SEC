  ...
  if (!profile) {
    return (
      <div className="p-8 text-center bg-card rounded-2xl border">
        <p className="text-muted-foreground">{t('users.not_found') || 'Utilisateur non trouvé.'}</p>
        <button onClick={() => navigate(-1)} className="mt-4 text-primary font-bold">{t('common.back') || 'Retour'}</button>
      </div>
    );
  }
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex items-center justify-between">
        ...
        >
          ...
          <span className="text-sm font-bold text-muted-foreground group-hover:text-foreground">{t('common.back') || 'Retour'}</span>
        </button>
              ...
              <div className="grid grid-cols-1 gap-4">
                {reviews.length > 0 ? (
                  ...
                  <div className="p-8 text-center bg-muted/10 rounded-2xl border border-dashed">
                    <p className="text-xs text-muted-foreground italic">{t('reviews.empty')}</p>
                  </div>
                )}
              </div>
            </div>
